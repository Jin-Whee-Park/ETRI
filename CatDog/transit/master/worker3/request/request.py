...
if __name__=="__main__":
    while(1):
        result = os.popen('ls -l /sender/videos/ | grep ^- | wc -l').read()
        if result == '1':
            continue
        else:
            filepath = "/sender/videos"
            ssh = SSh('IP', 'AccountName', 'Password')
            ssh.scp.put(filepath, recursive=True, remote_path='/home/ubuntu/mnt')
            time.sleep(5)
            removeAllfile(filepath)
            time.sleep(5)
            count = os.popen('ls -l /sender/videos/ | grep ^- | wc -l').read()
            if count != result:
                os.system('curl IP:PORT -o /sender/result/result.txt')
            else:
                continue
